...
pub trait Convex<T: Scalar>: Scalar {
    ...
    fn complement(self) -> Disjoint<T, Self>
    where
        Self: Sized,
    {
    ...
    fn difference(self, other: Self) -> Disjoint<T, Self>
    where
        Self: Sized,
    {
...
impl<T: Scalar, U: Convex<T> + Display> Display for Disjoint<T, U> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let intervals: Vec<String> = self.intervals.iter().map(|i| i.to_string()).collect();
...
impl<T: Scalar + PartialOrd + Bounded> Convex<T> for (T, T) {
        ...
        Self: Sized,
    {
        if self.0 == T::min_value() && self.1 == T::max_value() {
            ...
            ((T::min_value(), self.0), (self.1, T::max_value())).into()
        }
    }
...
impl<T: Scalar, U: Convex<T>> ContinuousSet<T, U> {
    ...
    pub fn union(self, other: impl Into<Self>) -> Self {
        match (self, other.into()) {
            ...
            (ConvexSet(c), DisjointSet(d)) | (DisjointSet(d), ConvexSet(c)) => d.union(c).into(),
        }
    }
